...
const PositionReset = () => {
  ...
  const clientState = useSelector(
    (state: any) => state?.client.clientDataformat
  );
  ...
  const isMultifilterOn = () => {
    return Object.values(clientState).map((ele: any) => {
  ...
  return (
    <div
      className={isMultifilterOn().includes(true) ? "cursor-pointer mr-2 mt-[2px]" : "cursor-not-allowed mt-[2px]"}
      onClick={() => reset()}
    >
      <svg
      ...
      >
        ...
        >
          <text
            ...
            fill={`${isMultifilterOn().includes(true) ? "#121820" : "#a4a5a5"}`}
            font-size="18"
          ...
          >
            <path
              ...
              fill={`${
                isMultifilterOn().includes(true) ? "#121820" : "#a4a5a5"
              }`}
            />
            <g
              id="Group_1255"
